...
#camcapture.set(cv.CV_CAP_PROP_FPS,60)
camcapture.set(cv.CV_CAP_PROP_FRAME_HEIGHT,480)
camcapture.set(cv.CV_CAP_PROP_FRAME_WIDTH,640)
...
while True:
    valid = camcapture.grab()
    
    if valid:
        valid, frame_mat = camcapture.retrieve() 
    else:
        break
    
    if frame_mat is None or not valid:
        break
...
#    cv.ShowImage('raw',frame)
    ...
    print str(len(image_string) / 8192.0)
